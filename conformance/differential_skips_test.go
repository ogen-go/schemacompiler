...
// is dropped before the `not` is ever reached, and it waits on #72 rather than on anything
// about negation.
//
...
// survive. Its fourth entry moved to #72 — that schema writes `properties` and
// `additionalProperties` with no sibling `type`, so the object shape is dropped before any
// field plan exists to carry.
//
...
var diffSkips = map[string]string{
	...
	"infinite-loop-detection.json :: evaluating the same schema location against the same data location twice is not a sign of an infinite loop :: failing case": "#72",
