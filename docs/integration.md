...
| `PropertyDispatch{Property, Cases, Tag}` | `Discriminator = Property`, `Mapping` built from `Cases` | Tagged union (design §18.2); this is ogen's explicit/implicit discriminator path. `Tag == TagDeclared` means an OpenAPI `discriminator` named the property (`handleExplicitDiscriminator`), `TagInferred` means it was recovered structurally (`implicitDiscriminatorKey`). A `TagDeclared` dispatch carries the same disjointness proof as an inferred one (design §18, §15.3): every branch **requires** `Property` and pins it to a const/enum, and the branch's cases cover every value it accepts. Declared cases come from `mapping` when it names the branch, else from the branch's own const/enum — never from the referenced component's name, which constrains nothing in the instance. A declaration that cannot drive dispatch — a `mapping` entry resolving to no branch or omitting a value its branch accepts, a `propertyName` some branch does not require or leaves unconstrained, or two branches sharing a value — is reported as a `SeverityWarning` diagnostic and the plan falls back to structural inference and then to `PredicateCountDispatch`, so a backend never switches on a value the author did not write and never gets a static dispatch that was not proven. |
