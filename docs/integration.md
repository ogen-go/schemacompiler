...
| `ArrayRepresentation{Prefix, Rest}` | `KindArray` when `Prefix` is empty; a tuple-as-struct (`KindStruct` with positional fields) when `Prefix` is non-empty, following ogen's existing `prefixItems` tuple lowering | Every `ItemRepresentation` carries a whole `CompilationPlan` for the values at that position, lowered by recursing through §1-§4, plus a `Presence` saying whether the slot may be absent (`prefixItems` applies only to the positions an instance has, so slot `i` is required iff `i < minItems`). A `Rest` over `NeverRepresentation` is `items: false` — a fixed-length tuple. There is no first-class ogen fixed-length-array kind, so treat it as a tuple struct with a validated length rather than a fixed-size Go array. |
...
| `LiteralDispatch{Cases}` | `ValueDiscriminators` | Enum/const union (design §18, discriminator class 2); each case becomes one entry in `ValueToVariant`. Build it from `LiteralCase.Raw`, the literal's exact source bytes: `Value` is a decoded convenience whose Go type is not canonicalized, so a number arrives as `int`, `uint64` or `float64` depending on its source text, and a value past float64's precision survives only in `Raw` (issue #152). |
