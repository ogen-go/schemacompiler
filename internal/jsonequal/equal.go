// Package jsonequal compares two JSON values for semantic equality (order-independent
// objects, exact numeric comparison via big.Rat). It is adapted from ogen's json.Equal
// (github.com/ogen-go/ogen, MIT); schemacompiler cannot import ogen directly because ogen
// depends on schemacompiler, so the primitive is vendored here. When the two merge, this
// can be replaced by ogen's json.Equal.
package jsonequal
...
// Equal reports whether a and b are the same JSON value.
