...
func TestBuild_SubSchemaPlansSurvive(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Len(t, sub.Validation.Predicates, 1)
			require.Equal(t, plan.MinLengthPredicate{Value: 1}, sub.Validation.Predicates[0].Expression)
			require.Equal(t, plan.SetString, sub.Validation.Predicates[0].Applicability)
		})
	}
}
